...
export const handler = async (event) => {
  ...
  const { secretKey: stripeKey, webhookSecret } = await loadStripeSecrets();
    ...
    if (eventType === 'payment_intent.payment_failed') {
      ...
      if (!orderId) return createSuccessResponse({ message: 'No orderId metadata; ignoring payment_failed' }, 200);
      const order = await orderModel.get(orderId);
      if (!order) return createSuccessResponse({ message: 'Order not found for failed payment (already removed?)' }, 200);
  ...
  // 3. Charge refunded -> only full refunds supported; partial refunds ignored/logged
    if (eventType === 'charge.refunded') {
      ...
      let orderId = null;
      if (paymentIntentId) {
        // Naive fallback lookup: if orderModel exposes a list or scan (placeholder; expensive in large tables)
        try {
          ...
          if (found && found.length === 1) {
            ...
            console.warn('Multiple orders found for paymentIntentId; using first', { paymentIntentId, count: found.length });
            order = found[0];
            orderId = order.id || order._id;
          }
        } catch (e) {
      ...
      if (!order) return createSuccessResponse({ message: 'Order not found for refund event' }, 200);
  ...
  const isFull = refunded >= amount;
      ...
      if (!isFull) {
        console.warn('Partial refund detected but feature disabled. Ignoring.', { orderId, refunded, amount });
        return createSuccessResponse({ message: 'Partial refund ignored (unsupported)' }, 200);
      }
      // Full refund path
  timeline = [...timeline, { at: now, type: 'refund_full', actor: 'system', actorType: 'system', actorId: 'system', meta: { chargeId: charge.id } } ];
      await orderModel.update(orderId, {
    ...
    if (eventType === 'account.updated') {
      ...
      const transfersActive = account.capabilities && account.capabilities.transfers && (account.capabilities.transfers === 'active' || account.capabilities.transfers.status === 'active');
