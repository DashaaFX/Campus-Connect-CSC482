...
const AdminProducts = () => {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const prodSubId = product.subcategory?._id || product.subcategory;

    if (selectedCategory && prodCatId !== selectedCategory) return false;
    if (selectedSubcategory && prodSubId !== selectedSubcategory) return false;
    return true;
  });
                ...
                <TableRow key={product._id}>
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-3">
                      {product.images?.[0] ? (
                        <img
                          src={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5011'}${product.images[0]}`}
                          alt={product.title}
