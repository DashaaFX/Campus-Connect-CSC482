...
export const register = async (req, res) => {
  try {
    const { fullname, email, phoneNumber, password, idnum, role } = req.body;

    if (!fullname || !email || !phoneNumber || !password || !role || !idnum) {
    ...
    const existingidnum = await User.findOne({ idnum });
    if (existingidnum) {
      return res.status(400).json({
